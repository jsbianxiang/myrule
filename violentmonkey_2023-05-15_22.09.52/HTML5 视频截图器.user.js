...
// @version      2.10.9
// @description  基于HTML5的简单原生视频截图，可控制快进/逐帧/视频调速，支持自定义快捷键。个人私用修改快进后退步进5秒
// @author       indefined（作者）、草木灰（修改者）
// @match        *://*/*
// @icon         https://cdn.icon-icons.com/icons2/188/PNG/256/Windows_Media_Player_22900.png
// @run-at       document-end
    ...
    **/
    ...
    function actionHandler(action, ev) {
        ...
        switch (action) {
            ...
            case 'forward':
                value = 5;
                if (ev.ctrlKey) value *= 3;
                if (ev.shiftKey) value *= 6;
                if (ev.altKey) value *= 5;
